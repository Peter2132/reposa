...
while True:
    ...
    print("6. Выбрать тариф")
    print("7. Сгенерировать чек")
    print("8. Отобразить все данные")
    print("0. Выйти")

    choice = input("Выберите операцию (введите соответствующую цифру): ")

    if choice == "1"and authenticated_role == "admin":
        tariff_manager.insert_tariff()
    elif choice == "2"and authenticated_role == "admin":
    ...
    elif choice == "3"and authenticated_role == "admin":
        tariff_id = input("Введите ID тарифа для удаления: ")
        tariff_manager.delete_tariff(tariff_id)
    elif choice == "4"and authenticated_role == "admin":
